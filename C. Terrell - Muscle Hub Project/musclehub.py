...
AND 
    visits.last_name = fitness_tests.last_name
AND
    visits.email = fitness_tests.email
LEFT JOIN
    applications
ON
    visits.first_name = applications.first_name
AND
...
LEFT JOIN
    purchases
ON
    visits.first_name = purchases.first_name
AND
    visits.last_name = purchases.last_name
AND
...
ab_counts = ab_counts.rename(columns = {"email":"counts"})
print ab_counts
...
plt.title('Percent of visitors in A/B Test Groups')
...
app_counts = app_counts.rename(columns={"email": "counts"})
...
member_counts = member_counts.rename(columns={"email": "counts"})
...
all_members = df.groupby(['ab_test_group', 'is_member']).email.count().reset_index()
all_members = all_members.rename(columns={'email':'counts'})

final_member_pivot = all_members.pivot(columns='is_member', index='ab_test_group', values='counts').reset_index()
...
plt.title('Applicant Purchases')

plt.savefig('app_purchases.png')
plt.show()
...
plt.title('All Visitor Purchases')

plt.savefig('all_visitor_purchases.png')
plt.show()
